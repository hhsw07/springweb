...
public class DownloadViewer extends AbstractView{	// Viewer를 container에 등록시켜 사용해야한다.
	// 현재 다운로드할 파일의 위치 (공통설정파일에서 호출) -> info에 작성되어있음
	@Value("${upload}")
	...
	protected void renderMergedOutputModel(
		...
		// 요청으로 온 file명은 model이름으로 downloadviewer한테 호출한다.
		...
		System.out.println("## viewer오신것을 환영합니다.");
		...
		// 		1) FileCopyUtils의 copy 메서드를 통해서 전달.
		FileCopyUtils.copy(fis, out);
		//		2) 전송 완료 처리
		out.flush();
// 		springweb.z02_util.DownloadViewer --> Viewer를 container에 등록시켜 사용해야한다.(dispatcher-servlet.xml)
...
2. 다운로드 처리
	...
	3) 반응 객체(response)로 전달 처리(클라이언트에 파일)
		...
		- setContentLength로 파일이름 길이 설정.
		- 한글명으로된 파일은 Encoding처리.
